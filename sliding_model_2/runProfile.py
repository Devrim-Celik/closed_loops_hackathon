...
def calc_target(state, profile, i, K, dt=0.005):
    ...
    def solver_dampingForce(state, profile):
        ...
        for n in range(0, N-1):
            ...
            if hasattr(i, '__len__'): # if we get sequence of all currents
                x[n+1] = ODE(x[n][0], x[n][1], x[n][2], x[n][3], Zh, Zh_dt, i[n], dt)
            else:
                x[n+1] = ODE(x[n][0], x[n][1], x[n][2], x[n][3], Zh, Zh_dt, i, dt)
            # Get the next profile
            Zh = profile[n+1]
            Zh_dt = (Zh - profile[n]) / dt
        return x

    x = solver_dampingForce(state, profile)
